...
#     logger.warning("Memory-efficient GCoder implementation unavailable: %s" % e)
#     Line = PyLine
...
def split(line):
    ...
    if not split_raw:
        ...
        line.is_move = False
        logger.warning("raw G-Code line \"%s\" could not be parsed" % line.raw)
        return [line.raw]
    command = split_raw[0]
...
class GCode(object):
    ...
    def __init__(self, data=None, home_pos=None, layer_callback=None, deferred=False):
        if not deferred:
