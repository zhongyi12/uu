...
function run(command, color, name) {
  ...
  child.stdout.on('data', data => {
    ...
    if (/Compiled/g.test(data.toString().trim().replace(/\n/g, `\n${repeat(' ', command.length + 2)}`)) && !isElectronOpen) {
      ...
      run('cross-env NODE_ENV=development BABEL_ENV=development electron src/main/index.dev.js', BLUE, 'electron');
      isElectronOpen = true;
    }
  });
...
run('cross-env NODE_ENV=development BABEL_ENV=renderer webpack-dev-server --hot --colors --config config/webpack.renderer.config.js', YELLOW, 'webpack');
