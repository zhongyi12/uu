...
class GcodeExport(inkex.Effect):
    ...
    def effect(self, svg_data=None, png_data=None):
        if os.path.isdir(self.directory):
            ...
            pos_file_png_exported = os.path.join(inkex.PNG_PATH, self.filename + ".png")
            pos_file_png_BW = os.path.join(inkex.PNG_PATH, self.filename + suffix + "preview.png")
            pos_file_gcode = os.path.join(inkex.LASERGCODE_PATH, self.filename + suffix + "gcode.gcode")
            self.pos_file_png_BW = pos_file_png_BW

            if svg_data is not None:
                pos_file_svg = os.path.join(inkex.SVG_PATH, self.filename + ".svg")
    ...
    def png2gcode(self, pos_file_png_exported=None, pos_file_png_BW=None, bytes=None):
        ...
        if self.preview_only == False:  # Genero Gcode solo se devo
            ...
            Scala = self.resolution * self.scale #0.54#0.51555
            ...
            gcode_data_list = []
            if self.homing == 1:
                # gcode_data = '\n'.join([gcode_data, 'G28; home all axes'])
                gcode_data_list.append('G28; home all axes')
            elif self.homing == 2:
                # gcode_data = '\n'.join([gcode_data, '$H; home all axes'])
                gcode_data_list.append('$H; home all axes')
            else:
                pass
            # gcode_data = '\n'.join([gcode_data,
            ...
            #     'G00 Z{0:.2f}'.format(self.z_home),
            # ])
            gcode_data_list.append('G21; Set units to millimeters')
            gcode_data_list.append('G90; Use absolute coordinates')
            gcode_data_list.append('G92; Coordinate Offset')
            gcode_data_list.append('G00 Z{0:.2f}'.format(self.z_home))
                            ...
                            if matrice_BN[y][x] == N:
                                if Laser_ON == False:
                                    gcode_data_list.append('G00 X{0:.2f} Y{1:.2f} F{2:.2f}'.format(
                                            ...
                                            -(float(x) / Scala),
                                            F_G00))
                                    # gcode_data_list.append(self.laseron)
                                    Laser_ON = True
            ...
            #         if y % 2 == 0:
            #             for x in range(w):
            #                 tmp = 255 - matrice_BN[y][x]
            #                 new_x = float(y) / Scala
            #                 new_y = -(float(x) / Scala)
            #                 if x == 0 and y == 0:
            #                     gcode_data_list.append('G00 X{0:.2f} Y{1:.2f} F{2}'.format(
            #                         new_x,
            #                         new_y,
            #                         F_G00)
            #                     )
            #                     # power = tmp
            #                 elif new_x != last_x or new_y != last_y:
            #                     power = tmp
            #                     if power > self.startpower:
            #                         gcode_data_list.append('G01 X{0:.2f} Y{1:.2f} F{2} P{3}'.format(
            #                             new_x,
            #                             new_y,
            #                             F_G01,
            #                             power)
            ...
            #                         new_x = float(y) / Scala
            #                         new_y = -(float(x) / Scala)
            #                         if new_x != last_x or new_y != last_y or (x == 0 and y == 0):
            #                             gcode_data_list.append('G00 X{0:.2f} Y{1:.2f} F{2}'.format(
            #                                 new_x,
            #                                 new_y,
            #                                 F_G00)
                        ...
                        for x in range(w):
                            if matrice_BN[y][x] != B:
                                if Laser_ON == False:
                                    ...
                                    power = 255 - matrice_BN[y][x]
                                    # power = power // 10 * 10
                                    # power = int(power * (255 - self.startpower) / 255) + self.startpower
                                    Laser_ON = True
            ...
            gcode_data_list.append('G00 X{0:.2f} Y{1:.2f} Z{2:.2f} F{3}'.format(
                    self.x_home,
                    self.y_home,
                    self.z_home,
                    F_G00))
            ...
            # # gcode_data = "\n".join([gcode_data, 'G00 X0 Y0; home'])
            # gcode_data = '\n'.join([gcode_data,
            #     'G00 X{0:.2f} Y{1:.2f} Z{2:.2f} F{3}'.format(
            #         self.x_home,
            #         self.y_home,
            #         self.z_home,
            #         F_G00)
