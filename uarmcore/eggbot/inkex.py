...
from .template import SVG_TEMPLATE
from .gcode import GcodeExport
...
try:
    ...
    localize()
    logger.error("The fantastic lxml wrapper for libxml2 is required by inkex.py and therefore this extension."
...
class Effect:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.doc_ids = {}
        self.ids = []
        self.__gcode_build = GcodeExport(**kwargs)
    ...
    def parse(self, filename=None):
        ...
        if filename is not None:
            try:
                ...
                logger.error("Unable to open specified file: %s" % filename)
                sys.exit()
        ...
        # an object member
            ...
            try:
                ...
                logger.error("Unable to open object member file: %s" % self.svg_file)
                sys.exit()
    ...
    def write_svg_to_file(self, data=None, path=None):
        if data is None and path is None:
            path = os.path.join(SVG_PATH, "template.svg")
            if isinstance(SVG_TEMPLATE, str):
                data = bytes(SVG_TEMPLATE, encoding='utf-8')
            else:
                data = SVG_TEMPLATE
    ...
    def write_gcode_to_file(self, data, path=None):
        if path is None:
            path = os.path.join(GCODE_PATH, "eggbot.gcode")
    ...
    def affect(self, text):
        """Affect an SVG document with a callback effect"""
        self.svg_file = self.write_svg_to_file()
        localize()
        # self.getoptions()
        self.parse()
        self.getposinlayer()
        self.effect(text)
        svg_data = etree.tostring(self.document)

        svg_path = os.path.join(SVG_PATH, "eggbot.svg")
        self.write_svg_to_file(svg_data, svg_path)

        gcode_data = self.__gcode_build.generate_gcode(svg_data)
        self.write_gcode_to_file(gcode_data)
    ...
    def xpathSingle(self, path):
        try:
            ...
            logger.error("No matching node for expression: %s" % path)
            retval = None
        return retval
    ...
    def getDocumentUnit(self):
        ...
        if viewboxstr:
            ...
            if p:
                ...
                logger.error("SVG Width not set correctly! Assuming width = 100")
            if u:
