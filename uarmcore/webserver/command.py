...
def get_error_msg(error_id, error_msg, error_code='0'):
    error_msg = {
        "status": False,
        "id": error_id,
        "error_msg": error_msg,
...
##################################### Grove #######################################
    def uarm_init_grove(self, client, id, data):
        ...
        if not gl.grove_sensors[pin]['init']:
            if gl.uarm.init_grove_sensor(pin, grove_id, value=values, wait=True):
                gl.grove_sensors[pin]['init'] = True
                gl.push_response_queue(client, get_response_msg(id))
            else:
                print ("E25")
                gl.push_response_queue(client, get_error_msg(id, "init grove failed", error_code='E25'))
        else:
            gl.push_response_queue(client, get_response_msg(id, "Already init this pin"))
    ...
    def __get_report_grove_sensor(self, client, pin):
        while True:
            if not pin in gl.grove_sensors:
                break
            if not gl.grove_sensors[pin]['init']:
                break
